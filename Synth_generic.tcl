...
#Analyse
analyze -format sverilog $netlist_serach_path/$netlist_name
...
if {[sizeof_collection [get_ports clk]]} {
    set clk_name ""
    set clk_port ""
...
set_clock_uncertainty "" [get_clocks $clk_name]
set_clock_transition "" [get_clocks $clk_name]

set_wire_load_mode ""

set typical_input_delay ""
set typical_output_delay ""
set_typical_wire_laod ""

#DRV settings
set_max_fanout "" $top_level

#Configure clock ntwrk
...
set_output_delay $typical_output_delay [all_outputs] -clock $cl_name
...
create_clock --- {create_clock -name sclk -period 0.932 -waveform {0 0.466} -add [get_ports { sclk } ] -comment clock_on_a_port
create_clock --- {create_clock -name pi_refclk -period 9.059 -waveform {0 4.5295} -add [get_ports { refclk } ] -comment clock_on_a_port
create_clock --- {create_clock -name noisy_ck_tp -period 45.353 -waveform {0 22.6765} -add [get_pins {ebg/ebg0/ana_rng_top_0/NOISY_CK_TP} ] -comment clock_on_a_pin
create_clock --- {create_clock -name ck_fast_tp -period 3.614 -waveform {0 1.807} -add [get_pins {ebg/ebg0/ana_rng_top_0/FAST_CK_DIV16_TP} ] -comment clock_on_a_pin
create_clock --- {create_clock -name brn_ck_tp -period 45.353 -waveform {0 22.6765} -add [get_pins {ebg/ebg0/ana_rng_top_0/BRN_CK_TP} ] -comment clock_on_a_pin
create_clock --- {create_clock -name brn_ck -period 45.353 -waveform {0 22.6765} -add [get_pins {ebg/ebg0/ana_rng_top_0/BRN_CK} ] -comment clock_on_a_pin
create_generated_clock --- {create_generated_clock {ebg/ebg0/ebg_ds0/DS_CL_GATE/QCK} -name clk_ds -master_clock brn_ck -source [get_pins ebg/ebg0/ana_rng_top_0/BRN_CK] -divide_by 1 -add


#DRV
set_default_constraints --- set_max_fanout 40 rnk
set_default_constraints --- set_max_transition 0.076 rnk
set_default_constraints --- set_max_capacitance 0.240 rnk
set_default_constraints --- set_max_area 0
...
source /mrvl2g/dc5purecl01_s_t103a0_wa_003/t103a0/t103a0/wa_003/jeffb/impl/rnk.PNR1/track.physconf40_0414_2nd_rnm_RTL_update/config/finish_constraints.tcl
...
write_sdc -nosplit /proj/t103a0/wa_003/jeffb/impl/rnk.PNR1/track.physconf40_0414_2nd_rnm_RTL_update/dc.syn/dataout/rnk.dc.syn.finish.func:max1.setup.sdc
...
report_area -hierarchy -designware >> ${top_level}.area.rpt
report_power -hier -analysis_effort medium >> ${top_level}.power.rpt
report_design
report_cell
report_port -verbose
report_compile_options
report_constraint -all_violators -verbose
report_timing -path full_path -delay max -max_paths $maxpaths -nworst 100
