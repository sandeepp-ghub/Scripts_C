...
proc GetPortXY {} {
    global cell;
    set x_mid [get_db $cell .location.x]
    set y_mid [get_db $cell .location.y]
#    echo $x_mid
#    echo $y_mid
    return "$x_mid $y_mid"
}
...
set mediumFlyLine 350;#150--275u
    ...
    if {[info exists results(-ports_list)]} {
        if {$results(-ports_list) ne ""} {
            set macros [get_db ports $results(-ports_list) -if  {.name!=VDD && .name!=VSS}]
            puts $macros
        } else { 
    ...
    if {[info exists results(-all)]} {
        set macros [get_db ports -if  {.name!=VDD && .name!=VSS}] ;#||[{.is_macro==true}]
    }
    ...
    foreach macro $macros {
        set cell $macro;
        puts $cell
        set macroFlyLineLnt [drew_macro_arrow];
        #-- collor macro.
...
define_proc_arguments pig  \
    ...
    -define_args {
        ...
        {-ports_list  "Get an inst: list of macros to run on"    "" list    optional}
...
#    set afiflops [add_to_collection $afiflops1 $afiflops2]
    set allFlops [add_to_collection $afo  $afi ]
    #-- filter *scan* flops.
#set allFlops [filter_collection $allFlops "name!~*_bist_*&&name!~*_csr_"]
#puts "[get_object_name $allFlops]"
    ...
    set XAV [expr {$XT/$i}]
    set YAV [expr {$YT/$i}]
    set_layer_preference red -color red -is_visible 1 -line_width 2
