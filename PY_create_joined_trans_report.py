...
pod2usage(msg => "No file given. For help use: create_joined_transition_report.pl --help\n" ,verbose => 2 ) if ((@ARGV < 1 ) && (-t STDIN));
...
################### 
foreach my $TransitionReportFileNname (<@ARGV>) {
	print "Reading transition report $TransitionReportFileNname ...\n";
	my @TransitionReport = read_file($TransitionReportFileNname);
	...
	my $in_rpt = 0;
	foreach my $line (@TransitionReport) {
		#print $line ;
		chomp($line);
		...
		if ($in_rpt > 0 && $index >= $in_rpt && (length($line) > 1)) {
		    ...
		    my @split_required	 = split(":",$split_line[10]);
			my $pin = "$split_line[6] ( $split_line[5] )";
			my $required =  $split_required[1];
			my $actual = $split_line[0];
			my $slack = $split_line[-1];
            # at 106/ccpd the slack is not writen as the last var of the line, temp fix for ccpd. 
            if ($slack == "set") {
			...
			$Local_DB{$pin}{"report_file"} = $TransitionReportFileNname;
			#$driver = $driver."   ";
...
print "Saving transition report to $OutFileName\n\n";
print INFOut "Joined transition report : \n\n";
print INFOut $tb;
close(INFOut);
...
The script read STA transition reports and create a joined report.
The script can read several STA reports

=head1 SYNOPSIS

create_joined_transition_report.pl [PrimeTime Transitions report] -o <OutputFileName>
create_joined_transition_report.pl ../reports/*tran_report_constraint.rpt.gz
...
The joined report filename. Shorted by transition violation.
