...
set port_file "../reports/dct/temp_in"
redirect  ../work/no_good_ports_in.tcl   {puts ""}
redirect  ../work/output_del_in.tcl      {puts ""}
redirect  ../work/internal_clock_driving_in_ports.rpt  {puts ""}
...
# ##########################################################
    set brk "0"
    set clkPortsCol [filter_collection [all_fanout -from $port -flat -endpoints_only -only_cells ] "name!=\*\*logic_0\*\* AND name!= \*\*logic_1\*\*"]
    set clkPortsCol [filter_collection [get_pins -of_object $clkPortsCol] "name==CK OR name==CP OR name==clocked_on"]
...
#     puts "1"
           ...
           redirect -append ../work/internal_clock_driving_in_ports.rpt { 
                pc $port;                                          
...
# ###############################################
       foreach_in_collection clockPort $clock_driver_ports_col {
            if {[compare_collection $oldClock $clockPort]=="0"} {
                ...
                redirect -append ../work/no_good_ports_in.tcl {puts "port $port is been drived by two clocks ports [get_object_name $oldClock] [get_object_name $clockPort]"; pc $clock_driver_ports_col}; 
                set brk 1; 
                break; 
            }
       }
...
# ######################################################
  ...
  set const "set_input_delay \[expr 0.65 * $clock_period\] -clock \[get_clocks [get_object_name $oldClock]\] \[get_ports $port\]"
  redirect -append ../work/output_del_in.tcl {puts $const}
}
