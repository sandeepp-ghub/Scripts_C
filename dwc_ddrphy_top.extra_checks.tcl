...
##-- Max skew across DfiClk inputs of all HardIP should be < 208ps
set DfiClkAllSkewLimit          0.208
...
## 1.1. Pclk Latency and skew for DBYTE
##------------------------------------------------------------------------------
##    - From acx4 PclkDbyte output pin to dbyte PclkIn input pins
##==============================================================================
##-- begin Pclk latency and skew for dbyte macros

echo ""
echo ""
echo "Latency delay from acx4 PclkIn pin to dbyte PclkIn input pins must be less than 280ps" 
echo "Skew between dbyte PclkIn pins must be less than 20ps " 
set endp   [get_pins {*u_DWC_DDRPHYDBYTE_*/PclkIn} -hier]
set num_endp [sizeof_collection $endp]
set startp [get_pins {*u_DWC_DDRPHYACX4_*/PclkIn}  -hier]
...
report_timing -path full_clock_expanded -through $startp -rise_to $endp -max 10000 -nworst 10000 -delay max -slack_lesser_than infinity > 1.1.pclk.rpt
echo "Reporting PClk latency and skew to the following $num_endp pins "
##-- report_latency_skew_endpoints $tp "PClk" ${PClkDBYTELatencyLimit} ${PClkSkewLimit}

report_latency_skew_notsp_notep $tp "PClk" "*u_DWC_DDRPHYACX4_*/PclkIn" "*u_DWC_DDRPHYDBYTE_*/PclkIn" ${PClkDBYTELatencyLimit} ${PClkSkewLimit}


##-- end Pclk latency and skew for dbyte macros
...
##==============================================================================
##-- begin Pclk latency and skew for acx4 macros

echo ""
echo ""
echo "Latency delay from master PClkOut output pins to acx4 PclkIn input pins must be less than 160ps" 
echo "Skew between acx4 PclkIn pins must be less than 20ps " 
set endp [get_pins {*u_DWC_DDRPHYACX4_*/PclkIn} -hier]
set num_pathp [sizeof_collection $endp]
set startp [get_pins {*u_DWC_DDRPHYMASTER_top/PclkOutC?} -hier ]
set tp     [get_timing_path -path full_clock_expanded -through $startp -rise_to $endp -max 10000 -nworst 10000 -delay max -slack_lesser_than infinity]
report_timing  -path full_clock_expanded -through $startp -rise_to $endp -max 10000 -nworst 10000 -delay max -slack_lesser_than infinity > 1.2.pclk.rpt
echo "Reporting PClk latency and skew to the following $num_pathp pins "
##-- report_latency_skew_endpoints $tp "PClk" ${PClkACX4LatencyLimit} ${PClkSkewLimit}

report_latency_skew_notendpoints $tp "PClk" "*u_DWC_DDRPHYACX4_*/PclkIn" ${PClkACX4LatencyLimit} ${PClkSkewLimit}


##-- end Pclk latency and skew for acx4 macros
...
echo "Skew between all dbyte and acx4 and master DfiClk pins must be less than 208ps" 
...
#set startp [get_port atpg_PllRefClk]
set startp [get_pins dwc_ddrphy_pnr_block/dwc_ddrphy_macro_0/atpg_PllRefClk]
report_timing -path full_clock_expanded -from $startp -rise_to $endp -delay max -slack_lesser_than infinity > 3.atpg_PllRefClk.rpt
...
report_timing -path_type full_clock_expanded -through $startp -rise_to $endp -delay max -slack_lesser_than infinity > 4.PllBypClk.rpt
...
report_timing -path full_clock_expanded -from $stp -rise_to $endp -delay max -slack_lesser_than infinity > 5.1_func_PllRefClk.rpt
...
report_timing -path full_clock_expanded -from $stp -rise_to $endp -delay max -slack_lesser_than infinity > 5.2_atpg_PllRefClk.rpt
set tp     [get_timing_path -path full_clock_expanded -from $stp -rise_to $endp -delay max -slack_lesser_than infinity]
...
#set startp [get_port DfiClk]
set startp [get_pins dwc_ddrphy_pnr_block/dss_clocks/dss_clocks_xclk/phy_ref_clk_mx/cn_clk_mux_v1_0_inst/clkmux2/Z]
report_timing -path full_clock_expanded -from $startp -rise_to $endp -max 10000 -nworst 10000 -delay max -slack_lesser_than infinity > 5.2_dfi_clk.rpt
set tp     [get_timing_path -path full_clock_expanded -from $startp -rise_to $endp -max 10000 -nworst 10000 -delay max -slack_lesser_than infinity]
