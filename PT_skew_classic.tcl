set find_clocks sclk
set find_clocks pi_tck
set find_clocks {pi_refclk pi_tck gen_pem0_pclk_div2_qlm0 sclk}
set find_clocks {QLM1_LN0_CM0_CLK_GSMODE_PLL3_DIV2 QLM1_LN1_CM0_CLK_GSMODE_PLL3_DIV2 QLM1_LN2_CM0_CLK_GSMODE_PLL3_DIV2 QLM1_LN3_CM0_CLK_GSMODE_PLL3_DIV2}

set ts [lindex [file split [file readlink inp/router/[get_object_name [current_design]]]] end]
...
foreach find_clock $find_clocks {
	...
	foreach blk [concat $pnr_blks $phys_partitions $grout_partitions] {
	  puts "INFO: Looking for $blk ..."
	  set cells [get_cells -quiet -hier -filter "ref_name==$blk && full_name!~*qlm* && full_name!~*/i_gsern_slm*"]
	  #set pins [sort_collection [get_pins -quiet -of $cells -filter "direction==in"] full_name]
	  ...
	  foreach_in_collection pin $pins {
	    ...
	    if {[sizeof_collection $loads] > 0 && [is_clock [index_collection $loads 0] $find_clock]} {
	      ...
	      set string_of([get_object_name $pin]) "[format "%0.2f" $avg_insertion],[get_object_name $pin],$pindir,[sizeof_collection $loads],[sizeof_collection $all_loads]"
