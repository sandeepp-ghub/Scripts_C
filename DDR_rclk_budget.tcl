set ifile [open /user/lioral/budget_max r]
...
while { [gets $ifile line] >= 0 } {
    ...
    set tp         [get_timing_path -from $port -to [get_clocks *rclk*]]
    if {$tp eq ""} {
...
#echo "$cur_clock + $period  -$cur_jit -$cur_unct - $cur_data - $slack - $setupt"
    ...
    set new_val [expr $cur_budgt + ($cur_slack - $slack) ] ;# slakc is neg
#     echo "$cur_clock + $period  -$cur_jit -$cur_unct - $cur_data - $slack  = $new_val"
    echo "$cur_budgt + \($cur_slack -$slack\) = $new_val" 
    lappend inc "set_input_delay $new_val -max -clock rclk \[get_ports $port\] ;# slack $slack "
